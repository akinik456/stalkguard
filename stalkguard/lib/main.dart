...
class _HomePageState extends State<HomePage> {
  ...
  // Color hysteresis state: 0=BLUE, 1=ORANGE, 2=RED
  ...
  static const Duration _motionCooldown = Duration(minutes: 3); // spam olmasın
  ...
  Color _colorForState(int s, {required bool trusted}) {
    ...
    if (s == 2) return const Color(0xFFFF3B30);
    if (s == 1) return const Color(0xFFFF9800);
  ...
  void _maybeRunMotionCheck(double risk) {
    // “B” stratejisi: sadece şüphe oluşunca
    if (!_scanning) return;
  ...
  Widget _buildDebugPanel() {
    ...
    final sorted = List<DeviceModel>.from(devices)
      ..sort((a, b) =>
          ThreatEngine.calculateRisk(b).compareTo(ThreatEngine.calculateRisk(a)));
    ...
    if (_motionChecking) {
      ...
      final meters = (_lastMotion!["meters"] as num).toDouble();
      final speedMps = (_lastMotion!["speed_mps"] as num?)?.toDouble() ?? 0.0;
      final kmh = speedMps * 3.6;
      motionLine = "Motion: ${meters.toStringAsFixed(1)}m • ${kmh.toStringAsFixed(1)} km/h";
    } else {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            "Devices: $total  •  High(>=75): $high  •  Alerts: $alerts",
          ...
          if (top != null) ...[
            const Text("Top Risk Device", style: TextStyle(color: Colors.white70, fontWeight: FontWeight.w600)),
            ...
            Text("Risk: ${ThreatEngine.calculateRisk(top).toInt()}%", style: const TextStyle(color: Colors.white60)),
            Text("Avg RSSI: ${top.avgRssi.toStringAsFixed(1)}", style: const TextStyle(color: Colors.white60)),
            Text("Variance: ${top.rssiVariance.toStringAsFixed(2)}", style: const TextStyle(color: Colors.white60)),
            Text("Seen: ${top.seenCount}", style: const TextStyle(color: Colors.white60)),
            Text("Age: ${DateTime.now().difference(top.firstSeen).inSeconds}s", style: const TextStyle(color: Colors.white60)),
          ] else ...[
  ...
  Future<void> _init() async {
    ...
    BleBridge.listen(
      onDevice: (data) {
        ...
        // risk dropped => clear notification + let red drop
